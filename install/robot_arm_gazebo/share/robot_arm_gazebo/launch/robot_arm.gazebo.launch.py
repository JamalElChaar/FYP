#!/usr/bin/env python3
"""
Launch Gazebo Ignition simulation with robot_arm.

This launch file sets up a complete ROS 2 simulation environment with Gazebo Ignition for
the robot_arm 6-joint robotic arm.
...
def generate_launch_description():
    """
    Generate a launch description for the Gazebo Ignition simulation.

    This function sets up all necessary parameters, paths, and nodes required to launch
    the Gazebo Ignition simulation with the robot_arm. It handles:
    ...
    # Get the install path for GZ_SIM_RESOURCE_PATH (parent of share directory)
    # This allows Gazebo to find meshes using package://robot_arm_description/meshes/
    install_share_path = os.path.dirname(os.path.dirname(pkg_share_description))
    ...
    declare_use_gazebo_cmd = DeclareLaunchArgument(
        name='use_gazebo',
        default_value='true',
        description='Flag to enable Gazebo Ignition')
    ...
    declare_world_cmd = DeclareLaunchArgument(
        ...
        description='World file name (e.g., empty.world)')
    ...
    # Set additional path for package:// URI resolution
    set_env_vars_meshes = AppendEnvironmentVariable(
        'GZ_SIM_RESOURCE_PATH',
        install_share_path)
    ...
    ld.add_action(set_env_vars_meshes)
